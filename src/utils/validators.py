...
class RequestValidator:
    ...
    def validate_basic_structure(request: Dict[str, Any]) -> None:
        ...
        if "cmd" not in request:
            raise ValidationError("Missing required field: cmd")

        if "request_data" not in request:
            raise ValidationError("Missing required field: request_data")
            
        # Validate cmd field type
        if not isinstance(request["cmd"], int):
            raise ValidationError("Invalid cmd field: must be integer")
    ...
    def validate_command_code(cmd: int) -> None:
        ...
        Raises:
            ValidationError: If command code is invalid
        """
        valid_commands = {
    ...
    def validate_request(cls, request: Dict[str, Any]) -> None:
        ...
        if cmd == 0x01:  # request_classification
            ...
            if request_data and len(request_data) == 1:
